...
percRVotesL <- ElectionDataEDA %>% 
  dplyr::select(geograhic.region, pg2008, pg2012, pg2016) %>% group_by(geograhic.region) %>% summarise('2008' = mean(pg2008, na.rm = TRUE),
                                                                                                       '2012' = mean(pg2012, na.rm = TRUE),
                                                                                                       '2016' = mean(pg2016, na.rm = TRUE)) %>%
...
# Feature Selection
################################################################
lassoRegressedFeatures <- ElectionData[sapply(ElectionData, is.numeric)]
